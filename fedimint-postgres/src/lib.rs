...
use deadpool_postgres::{Config, Manager};
use fedimint_core::db::{
    IDatabaseTransactionOps, IDatabaseTransactionOpsCore, IRawDatabase, IRawDatabaseTransaction,
    PrefixStream,
};
use fedimint_core::util::backoff_util::{self, aggressive_backoff};
use fedimint_core::util::retry;
use futures::stream;
use tracing::debug;

use crate::envs::FM_POSTGRES_URL_ENV;
...
#[derive(Debug)]
pub struct PostgresDb(deadpool_postgres::Pool);
...
impl PostgresDb {
    pub fn create_pool(config: Config) -> anyhow::Result<PostgresDb> {
...
impl<'a> fmt::Debug for PostgresTransaction<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("PostgresTransaction")
    }
}
...
impl IRawDatabase for PostgresDb {
    ...
    async fn begin_transaction<'a>(&'a self) -> PostgresTransaction {
        let mut conn = retry(
            "Getting gateway info via gateway-cli info",
            backoff_util::aggressive_backoff(),
            || async { self.inner().get().await.map_err(anyhow::Error::from) },
        )
        .await
        .expect("Failed to get connection from pool");
        let tx = conn.transaction().await.expect("Failed to get transaction");
        PostgresTransaction { tx, _conn: conn }
    }
