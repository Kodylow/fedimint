...
# This file sets up the environment for running Fedimint and its associated services
# Important: This script assumes you have a domain under your control for DNS records

FEDIMINT_VERSION=0.2.2
echo "Welcome to the Fedimint installer."
echo "Using the latest stable version: $FEDIMINT_VERSION"
...
if docker compose version | grep 'Docker Compose' >&/dev/null; then
    ...
    if [ "$EUID" -ne 0 ]; then
        echo 'Error: docker-compose is not installed and cannot be installed for you.' >&2
        exit 1
    fi
    ...
    if [ -x "$(command -v apt)" ]; then
        ...
        echo 'Error: docker-compose is not installed and could not be installed for you.' >&2
        exit 1
    fi
fi
...
# Service selection
echo "Now, let's configure the services you want to set up."

# Fedimint Guardian Server
read -p "Do you want to set up the Fedimint Guardian Server as part of a t/n Federation? [Y/n] " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    SERVICES+=" fedimintd"
fi
...
# Lightning Gateway
read -p "Do you want to set up the Lightning Gateway service? (Note: This should be a different entity than the Fedimint Guardian Server!) [Y/n] " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    SERVICES+=" gatewayd"
    # LND Node
    read -p "Do you want to attach an LND node to the gateway? (Warning: Pointing to a remote LND node is untested and beta) [Y/n] " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        SERVICES+=" lnd"
    fi
...
select NETWORK in "bitcoin" "testnet" "signet" "mutinynet"; do
    case $NETWORK in
    bitcoin) break ;;
    testnet) break ;;
    signet) break ;;
...
# Bitcoin node or Esplora
read -p "Do you want to run a Bitcoin node on the same box? [Y/n] " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    SERVICES+=" bitcoind"
else
    read -p "Do you want to use a remote Bitcoin node or an Esplora client? [node/esplora] " -r
    if [[ $REPLY == "esplora" ]]; then
        read -p "Enter the Esplora API URL: " -r ESPLORA_API_URL
        SERVICES+=" esplora"
    elif [[ $REPLY == "node" ]]; then
        SERVICES+=" remote_bitcoin_node"
    fi
fi

# Get external IP and remote user
...
echo
read -p "Do you want to set up TLS with Let's Encrypt? [Y/n] " -n 1 -r -a setup_tls </dev/tty
echo
if [[ ${setup_tls[*]} =~ ^[Yy]$ ]]; then
    echo "Setting up TLS with Let's Encrypt..."
    setup_tls
else
    echo "Skipping TLS setup. Proceeding without TLS..."
fi

echo "Selected services: $SERVICES"
echo "Network selected: $NETWORK"
if [[ $SERVICES == *"esplora"* ]]; then
    echo "Esplora API URL: $ESPLORA_API_URL"
fi

# Confirmation before proceeding
echo
read -p "Press enter to confirm and proceed with the installation, or CTRL+C to abort." -r
echo
...
DOCKER_COMPOSE_FILE=https://raw.githubusercontent.com/kodylow/fedimint/docker/full-tls/docker-compose.yaml

echo "Downloading docker-compose file..."
download $DOCKER_COMPOSE_FILE ./docker-compose.yaml
echo "Replacing host names in docker-compose file..."
